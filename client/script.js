...
sayHelloButton.addEventListener('mouseleave', function() {
  sayHelloButton.style.backgroundColor = 'pink';
})
...
const repeatMyParam = () => {
  //YOUR CODE HERE
  axios.get('http://localhost:3000/repeat/{astring}')
    .then((res) => {
...
function query() {
  axios.get('http://localhost:3000/query-test/?query1&query2')
    .then((res) => {
      console.log(res.data);
    })
}
...
function createFood(event) {
  ...
  const foodInput = document.querySelector('#foodItem');
  console.log('food input value is: ' + foodInput.value);
  const body = {newFood : foodInput.value};
  console.log('body newFood has the value: ' + body.newFood);
  axios.post('http://localhost:3000/food/', body)
    .then((res) => {
      console.log(res.data);
      const foodItem = document.createElement('li');
      foodItem.textContent = res.data[res.data.length - 1];
...
const addFoodButton = document.querySelector('#addFoodBtn');
addFoodButton.addEventListener('click', createFood);